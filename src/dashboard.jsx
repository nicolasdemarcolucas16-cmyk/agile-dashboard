...
export default function Dashboard() {
    ...
    fetch(CSV_URL + '&cache=' + Date.now())
      ...
      .then((text) => {
        ...
        const body = lines
          ...
          .filter(Boolean)
          .map(parseCsvLine)
          .filter((r) => r && r.length > 5 && (r[1] || '').trim() !== '');
  ...
  const current = useMemo(() => {
    ...
    const withMonth = rows.filter((r) => r.mes && String(r.mes).trim() !== '');
    if (!withMonth.length) return null;

    return withMonth[withMonth.length - 1];
  }, [rows]);
            ...
            <div className="card">
              ...
              <div className="detail-list">
                ...
                <div>
                  ...
                  <strong>
                    ${safeNumber(current.pagosChofer).toLocaleString('es-AR')}
                  </strong>
                </div>
                <div>
                  <span>Gastos del vehículo</span>
                  <strong>
                    ${safeNumber(current.gastosVehiculo).toLocaleString('es-AR')}
                  </strong>
                </div>
                <div>
                  <span>Ganancia neta mensual</span>
                  <strong>
                    ${safeNumber(current.gananciaNetaMensual).toLocaleString('es-AR')}
                  </strong>
                </div>
                <div>
                  <span>Valor día</span>
                  <strong>
                    ${safeNumber(current.valorDia).toLocaleString('es-AR')}
                  </strong>
                </div>
              </div>
            </div>
          ...
          <div className="card">
            ...
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.mes || Math.random()}>
                      <td>{row.mes || '-'}</td>
                      <td>{safeNumber(row.diasTrabajados)}</td>
                      <td>{safeNumber(row.rendimiento).toFixed(2)}%</td>
                      <td>
                        ${safeNumber(row.gastosVehiculo).toLocaleString('es-AR')}
                      </td>
                      <td>
                        ${safeNumber(row.gananciaMensual).toLocaleString('es-AR')}
                      </td>
                    </tr>
                  ))}
                </tbody>
